...
export default function AddRolePage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const data = await postData("/role", { name, isActive });
    setLoading(false);
    ...
    const result = await alert("success", "บันทึกสำเร็จ", "เพิ่มประเภทผู้ใช้งานเรียบร้อยแล้ว");
  ...
  const handleCancel = async () => {
    ...
    if (result.isConfirmed) router.push("/roles");
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="w-full mx-auto">
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-sm text-gray-400">เพิ่มประเภทผู้ใช้งาน</span>
        </div>

        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">

          <div className="px-6 py-4 border-b border-gray-50">
            <p className="text-base font-medium text-gray-800">เพิ่มประเภทผู้ใช้งาน</p>
            <p className="text-xs text-gray-400 mt-1">กรอกข้อมูลประเภทผู้ใช้งานใหม่</p>
          </div>
