...
export default function UsersPage() {
  ...
  const fetchRoles = async () => {
    const data = await getData("/role?isActive=1&page=1&pageSize=100");
    if (data?.statusCode === 200) setRoles(data.data.items);
  };

  const fetchUsers = async (overrideSearch?: string, overrideRole?: string, overrideIsActive?: string, overridePage?: number) => {
    setLoading(true);
    ...
    const data = await getData(
      `/user?name=${currentSearch}&role=${currentRole}&isActive=${currentIsActive}&page=${currentPage}&pageSize=${pageSize}`
    );
  ...
  const handleDelete = async (id: number) => {
    const resultConfirm = await alertConfirm("warning","ยืนยันการลบ","ต้องการลบผู้ใช้งานนี้ไหม?");
    ...
    if (data?.statusCode === 200) {
      ...
      if (result.isConfirmed) fetchUsers();
    } else {
      ...
      <div className="w-full mx-auto">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 mb-4 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-medium text-gray-700">ค้นหาผู้ใช้</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                placeholder="พิมพ์ชื่อ หรือ Email ที่ต้องการค้นหา"
                autoComplete="off"
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block">ประเภทผู้ใช้</label>
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-50">
            <div>
              <p className="text-sm font-medium text-gray-800">รายการผู้ใช้งานในระบบ</p>
            </div>
            <button
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b border-gray-50 bg-[#1677ff]">
                  ...
                  <th className="text-center px-5 py-3 text-xs font-medium text-white">ชื่อผู้ใช้</th>
                  <th className="text-center px-5 py-3 text-xs font-medium text-white">Email</th>
                  <th className="text-center px-5 py-3 text-xs font-medium text-white">ประเภทผู้ใช้</th>
              ...
              <tbody>
                ...
                {!loading && users.map((user, index) => (
                  <tr key={user.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-3.5 text-black">{user.name}</td>
                    <td className="px-5 py-3.5 text-black">{user.email}</td>
                    <td className="px-5 py-3.5 text-center">{getRoleBadge(user.roleName)}</td>
                    <td className="px-5 py-3.5 text-center">{getStatusBadge(user.isActive)}</td>
                    <td className="px-5 py-3.5 text-black text-xs">{user.createdDate}</td>
