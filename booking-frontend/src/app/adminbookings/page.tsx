...
export default function AdminBookingsPage() {
  ...
  const fetchBookings = async (overrideName?: string, overrideBookingDate?: dayjs.Dayjs | null, overrideStatus?: string, overridePage?: number) => {
    setLoading(true);
    ...
    const data = await getData(
      `/booking?name=${currentName}&status=${currentStatus}&bookingDate=${currentBookingDate}&page=${currentPage}&pageSize=${pageSize}`
    );
      ...
      <div className="w-full mx-auto">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 mb-4 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-medium text-gray-700">ค้นหารายการจอง</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block">ชื่อผู้จอง</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSearch()}
                placeholder="พิมพ์ ชื่อผู้จอง"
                autoComplete="off"
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-50">
            <p className="text-sm font-medium text-gray-800">รายการจองทั้งหมด</p>
          </div>
            ...
            <table className="w-full text-sm min-w-[800px]">
              <thead>
                <tr className="bg-[#1677ff]">
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">เลขที่จอง</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">ชื่อผู้จอง</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">สนาม</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">วันที่จอง</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">ช่วงเวลา</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">ราคา</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">สถานะ</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-white">วันที่สร้าง</th>
                  <th className="text-center px-6 py-3 text-xs font-medium text-white w-24">จัดการ</th>
                </tr>
              </thead>
              <tbody>
                {bookings.length === 0 && (
                ...
                {bookings.map((booking) => (
                  <tr key={booking.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                    <td className="px-6 py-3.5 text-black text-xs font-medium">{booking.bookingNo}</td>
                    <td className="px-6 py-3.5 text-black text-xs">{booking.name}</td>
