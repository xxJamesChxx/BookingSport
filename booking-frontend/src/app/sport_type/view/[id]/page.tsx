...
export default function ViewSportTypePage() {
  ...
  const fetchSportType = async () => {
    const data = await getData(`/sporttype/${id}`);
    if (data?.statusCode === 200) {
      setSportType(data.data);
    } 
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="w-full mx-auto">
        ...
        <div className="flex items-center gap-2 mb-5">
          <Link href="/sport_type" className="flex items-center gap-1.5 text-sm text-[#1677ff] hover:text-blue-700">
          ...
          <span className="text-sm text-gray-400">รายละเอียดประเภทกีฬา</span>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            <div>
              <p className="text-base font-medium text-gray-800">รายละเอียดประเภทกีฬา</p>
            </div>
            <button
              onClick={() => router.push(`/sport_type/edit/${id}`)}
          ...
          <div className="px-6 py-6">
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-gray-400 mb-1">ชื่อประเภทกีฬา</p>
                  <p className="text-sm font-medium text-gray-800">{sportType?.name}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400 mb-1">สถานะ</p>
                  {sportType?.isActive === 1 ? (
              ...
              <div className="border-gray-50">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-700">{sportType?.createdDate || "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 mb-1">สร้างโดย</p>
                    <p className="text-sm text-gray-700">{sportType?.createdUser || "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 mb-1">วันที่แก้ไขล่าสุด</p>
                    <p className="text-sm text-gray-700">{sportType?.updatedDate || "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 mb-1">แก้ไขโดย</p>
                    <p className="text-sm text-gray-700">{sportType?.updatedUser || "—"}</p>
                  </div>
                </div>
              </div>
