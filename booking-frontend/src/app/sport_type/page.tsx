...
export default function SportTypePage() {
  ...
  const fetchSportTypes = async (overrideSearch?: string, overrideIsActive?: string, overridePage?: number) => {
    setLoading(true);
    ...
    const data = await getData(
      `/sporttype?name=${currentSearch}&isActive=${currentIsActive}&page=${currentPage}&pageSize=${pageSize}`
    );
    if (data?.statusCode === 200) {
      setSportType(data.data.items);
  ...
  const handleDelete = async (id: number) => {
    const resultConfirm = await alertConfirm("warning","ยืนยันการลบ","ต้องการลบประเภทกีฬานี้ไหม?");
    if (!resultConfirm.isConfirmed) return;

    const data = await deleteData(`/sporttype/${id}`);

    if (data?.statusCode === 200) {
      const result = await alert("success","ลบสำเร็จ");
      if (result.isConfirmed) fetchSportTypes();
    } else {
      ...
      <div className="w-full mx-auto">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 mb-4 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-medium text-gray-700">ค้นหาประเภทกีฬา</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block">ชื่อประเภทกีฬา</label>
              <input
                type="text"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSearch()}
                placeholder="พิมพ์ชื่อ ประเภทกีฬา ที่ต้องการค้นหา"
                autoComplete="off"
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-50">
            <div>
              <p className="text-sm font-medium text-gray-800">รายการประเภทกีฬาในระบบ</p>
            </div>
            <button
              onClick={() => router.push("/sport_type/add")}
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b border-gray-50 bg-[#1677ff]">
                  ...
                  <th className="text-center px-5 py-3 text-xs font-medium text-white">ชื่อประเภทกีฬา</th>
              ...
              <tbody>
                ...
                {!loading && sportType.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-16 text-sm text-gray-400">
                      ไม่พบข้อมูลประเภทกีฬา
                    </td>
                  </tr>
                )}

                {!loading && sportType.map((sport_type, index) => (
                  <tr key={sport_type.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                    <td className="px-5 py-3.5 text-black text-xs text-center">
                      {(page - 1) * pageSize + index + 1}
                    </td>
                    <td className="px-5 py-3.5 text-black">{sport_type.name}</td>
                    <td className="px-5 py-3.5 text-center">{getStatusBadge(sport_type.isActive)}</td>
                    <td className="px-5 py-3.5 text-black text-xs">{sport_type.createdDate}</td>
                    <td className="px-5 py-3.5 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          onClick={() => router.push(`sport_type/view/${sport_type.id}`)}
                        ...
                        <button
                          onClick={() => router.push(`/sport_type/edit/${sport_type.id}`)}
                        ...
                        <button
                          onClick={() => handleDelete(sport_type.id)}
