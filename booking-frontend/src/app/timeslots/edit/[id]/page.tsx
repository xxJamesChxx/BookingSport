...
export default function EditTimeSlotPage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const data = await putData(`/timeslot/${id}`, {
      startTime,
      endTime,
      isActive,
    });
    ...
    const result = await alert("success", "บันทึกสำเร็จ", "แก้ไขช่วงเวลาเรียบร้อยแล้ว");
    if (result.isConfirmed) {
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="w-full mx-auto">
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="text-sm text-gray-400">แก้ไขช่วงเวลา</span>
        </div>

        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">

          <div className="px-6 py-4 border-b border-gray-50">
            <p className="text-base font-medium text-gray-800">แก้ไขช่วงเวลา</p>
            <p className="text-xs text-gray-400 mt-1">แก้ไขข้อมูลช่วงเวลา</p>
          </div>
